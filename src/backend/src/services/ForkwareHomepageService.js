...
/*
 * Copyright (C) 2024-present Forkware Technologies Inc.
 *
 * This file is part of Forkware.
 *
 * Forkware is free software: you can redistribute it and/or modify
...
/**
 * ForkwareHomepageService serves the initial HTML page that loads the Forkware GUI
 * and all of its assets.
 */
class ForkwareHomepageService extends BaseService {
    static MODULES = {
        fs: require('node:fs'),
    }
    ...
    /**
    * @description This method initializes the ForkwareHomepageService by loading the manifest file.
    ...
    * @returns {Promise} A promise that resolves with the initialized ForkwareHomepageService instance.
    */
    async _init () {
        // Load manifest
    ...
    /**
    * This method sends the initial HTML page that loads the Forkware GUI and its assets.
    */
    async send ({ req, res }, meta, launch_options) {
        const config = this.global_config;
        
        if (
            req.query['forkware.app_instance_id'] ||
            req.query['error_from_within_iframe']
        ) {
            const easteregg = [
                'forkware in forkware?',
                'Infinite recursion!',
                'what\'chu cookin\'?',
            ];
        ...
        return res.send(this.generate_forkware_page_html({
            env: config.env,
    ...
    <head>
        ...
        <script>
            if ( ! window.service_script ) {
                ...
                * It is called after the ForkwareHomepageService instance has been constructed and initialized.
                *
                * @param {import('express').Request} req - The Express request object.
                * @param {import('express').Response} res - The Express response object.
                * @param {object} meta - Metadata about the Forkware instance, including the environment, manifest, and launch options.
                */
                // Add this comment above line 240
                // method: send
                // purpose: Send the initial HTML page that loads the Forkware GUI and its assets.
    ...
    <body>
        <script>window.forkware_gui_enabled = true;</script>
        ${
        ...
        <script type="module">
        /**
        * This method generates the HTML for the initial Forkware page, including script tags and other necessary metadata.
        ...
        *  - manifest: The Forkware GUI manifest
