...
router.all('*', async function(req, res, next) {
    ...
    // --------------------------------------
        ...
        // ------------------------
            ...
            const manifest =
                _fs.existsSync(_path.join(config.assets.gui, 'forkware-gui.json'))
                    ? (() => {
                        const text = _fs.readFileSync(_path.join(config.assets.gui, 'forkware-gui.json'), 'utf8');
            ...
            // index.js
            if(path === '/'){
                const svc_forkwareHomepage = Context.get('services').get('forkware-homepage');
                return svc_forkwareHomepage.send({ req, res }, {
                    title: app_title,
                    ...
                    company: 'Forkware Technologies Inc.',
    ...
    //------------------------------------------
    // User-defined subdomains: *.forkware.io
