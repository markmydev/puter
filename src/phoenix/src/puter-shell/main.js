...
export const launchPuterShell = async (ctx) => {
    ...
    ctx.externs.out.write(
        ...
        // '\n' +
        // `🔗  ${mklink('https://forkware.io', 'forkware.io')} ` +
        ''
        // `🔗  ${mklink('https://forkware.io', 'forkware.io')} ` +
    );
    ...
    if ( ! config.hasOwnProperty('puter.auth.token') ) {
        ...
        ctx.externs.out.write(
            `\x1B[33;1m⚠\x1B[0m` +
            `\x1B[31;1m` +
            ' You are not running this terminal or shell within forkware.io\n' +
            `\x1B[0m` +
            'Use of the shell outside of forkware.io is still experimental.\n' +
